...
BombingTank::BombingTank(Vec2 pos, TEAM team)
{
	m_BombingTank = new Animation();
	m_BombingTank->AddContinueFrame(L"Painting/Object/Tank/BombingTank", 1, 1);
	m_BombingTank->Init(7, true);
	m_BombingTank->SetParent(this);
	m_Position = pos;

	m_Barrel = Sprite::Create(L"Painting/Object/Tank/TankBarrel.png");
...
void BombingTank::OnCollision(Object* other)
{
	if (other->m_Tag == "Tile")
	{
		GroundCol(other);
...
void BombingTank::Shot()
{
	...
	if (INPUT->GetKey(VK_SPACE) == KeyState::UP)
	{
		if (Player::GetInst()->m_Power > 0)
			ObjMgr->AddObject(new Smoke(Player::GetInst()->m_Power, m_Barrel->m_Rotation, m_BarrelEnd, Team), "CannonBall");
		Player::GetInst()->m_Power = 0;
	}
}

void BombingTank::BarrelAngleContorl()
{
...
void BombingTank::GroundCol(Object* other)
{
	m_isGround = true;
...
void BombingTank::SetObjectPos()
{
	m_Barrel->m_Position.x = m_Position.x - 110 + cos(m_Rotation) * m_Size.x;
	m_Barrel->m_Position.y = m_Position.y - 35;
	m_Active->m_Position = m_Position;
	m_Active->m_Rotation = m_Rotation;
}

void BombingTank::SetVar()
{
	m_isGround = false;
...
float BombingTank::DotToLineDistance(Vec2 Dot, Vec2 Line1, Vec2 Line2)
{
